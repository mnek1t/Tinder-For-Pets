...
import dislikeIcon from "../assets/close-icon.svg";
...
const RecommendationCardSwipe = (props: CardSwipeProps) => {
  ...
  function handleButtonClick(isLike: boolean) {
    const swipeDistance = isLike ? 300 : -300;
    const duration = 0.3; 

    animate(motionValue, swipeDistance, {
      duration,
      onComplete: () => {
        props.handleSwipe(props.id, isLike);
        props.setCards((prevState) => prevState.filter((card) => card.id !== props.id));
      },
    });
  }
